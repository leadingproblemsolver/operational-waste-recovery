...
def ingest_jsonl(
    ...
    """Known-bad mutant: valid rows are persisted before strict validation completes."""
    path = Path(path)
    if not path.is_file():
    ...
    with path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            try:
                ...
                # Intentional mutation under evaluation: persistence occurs too early.
                store.insert(event)
                accepted_count += 1
    ...
    counts["inserted"] = accepted_count
